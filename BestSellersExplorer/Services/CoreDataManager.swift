...
class CoreDataManager: CoreDataManagerProtocol {
    ...
    func deleteFavoriteBook(by primaryIsbn13: String, completion: (Result<Void, Error>) -> Void) {
        ...
        if let bookToDelete = similarBooks.first {
            ...
            let error = NSError(domain: "CoreData", code: 404, userInfo: [NSLocalizedDescriptionKey: String(localized: "Book_with_ISBN_\(primaryIsbn13)_not_found_CoreData_deleteFavotiteBooks.")])
            completion(.failure(error))
        }
    }
}
